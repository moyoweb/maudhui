...
<module title="Aside" position="aside" prepend="0" xmlns="http://www.w3.org/1999/html">
    <a class="btn" href="<?= @route('view=conversation&parent='.$article->id); ?>"><?= @text('Discuss this Blog Post'); ?></a>
</module>
        ...
        <div class="row">
            <? if($article->isRelationable()) : ?>
                <? foreach($article->getConversations(array('limit' => 4)) as $conversation) : ?>
                    <? if($conversation->id !== $article->id && $conversation->isElementable()) : ?>
